...
def drawBoxesRGB(frame, rgb_value):
    ...
    cv2.rectangle(frame, (box_start_x, box_start_y),
                  (blue_box_end_x, box_end_y), color=[255, 0, 0], thickness=2, lineType=cv2.LINE_AA)

    blue_box_text = f"B: {rgb_value[2]}"
    drawTextBox(frame, blue_box_text, (
                blue_box_start_x, box_start_y - 40), 1, 2, [0, 0, 0])
    drawText(frame, blue_box_text, (blue_box_start_x,
             box_start_y - 40), [255, 0, 0], 1, 2)
...
def fillBoxesWithFingerRGB(frame, rgb_box_points, finger_box_coordinates):
